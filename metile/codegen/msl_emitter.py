...
def _emit_tensor_ops_gemm(func: mir.MFunction) -> str:
    """Generate optimized MSL for Metal 4 matmul2d.

    Two execution modes:
    - Preemptive (default): each SG independently runs matmul2d on its own subtile
    - Cooperative: all SGs work together on the full tile via execution_simdgroups<N>
    """
    gemm_op = None
    for op in func.ops:
        if isinstance(op, mir.MTensorOpsGemm):
            gemm_op = op
            break
    assert gemm_op is not None

    if gemm_op.cooperative:
        return _emit_tensor_ops_cooperative(func, gemm_op)
    return _emit_tensor_ops_preemptive(func, gemm_op)


def _emit_tensor_ops_header(
    func: mir.MFunction, gemm_op: mir.MTensorOpsGemm, need_sgid: bool
) -> list[str]:
    """Emit common header: includes, signature, tensor views."""
    in_type = gemm_op.in_type
    out_type = gemm_op.out_type
    ...
    # Tensor views
    pad = "    "
    a_name = gemm_op.ptr_a.name
    b_name = gemm_op.ptr_b.name
    c_name = gemm_op.ptr_c.name
...
def _emit_tensor_ops_epilogue(gemm_op: mir.MTensorOpsGemm, lines: list[str]):
    """Emit epilogue + store on cooperative_tensor."""
    pad = "    "
    if gemm_op.epilogue_ops:
        ...
        for epi in gemm_op.epilogue_ops:
            if epi[0] == "relu":
        ...
        lines.append(f"{pad}}}")
        lines.append("")
    lines.append(f"{pad}cT.store(mC);")


def _emit_ct_decl(lines, pad, in_type, suffix=""):
    """Emit cooperative_tensor input/output declarations."""
    s = suffix
    lines.append(
        f"{pad}auto ct_a{s} = op.get_left_input_cooperative_tensor"
        f"<{in_type}, {in_type}, {in_type}>();"
    )
    lines.append(
        f"{pad}auto ct_b{s} = op.get_right_input_cooperative_tensor"
        f"<{in_type}, {in_type}, {in_type}>();"
    )


def _emit_ct_zero_init(lines, pad, acc_type, ct_name="cT"):
    """Emit zero-initialization of destination cooperative_tensor."""
    lines.append(f"{pad}#pragma clang loop unroll(full)")
    lines.append(f"{pad}for (uint16_t i = 0; i < {ct_name}.get_capacity(); ++i) {{")
    lines.append(f"{pad}    if ({ct_name}.is_valid_element(i)) {ct_name}[i] = {acc_type}(0);")
    lines.append(f"{pad}}}")


def _emit_ct_load(lines, p, buf, k_expr, bk_inner, SM, SN):
    """Emit a single cooperative_tensor load pair for a 32-wide tile."""
    lines.append(f"{p}ct_a{buf}.load(tA.template slice<{bk_inner}, {SM}>({k_expr}, tile_row));")
    lines.append(f"{p}ct_b{buf}.load(tB.template slice<{SN}, {bk_inner}>(tile_col, {k_expr}));")


def _emit_separated_simple(lines, pad, in_type, acc_type, SM, SN, bk_inner, bk_outer):
    """Emit the non-pipelined separated load/compute K-loop."""
    lines.append(
        f"{pad}// Separated load/compute: cooperative_tensor inputs for load/compute overlap"
    )
    _emit_ct_decl(lines, pad, in_type)
    lines.append(f"{pad}auto cT = op.get_destination_cooperative_tensor<")
    lines.append(f"{pad}    decltype(ct_a), decltype(ct_b), {acc_type}>();")
    _emit_ct_zero_init(lines, pad, acc_type)
    lines.append("")

    lines.append(f"{pad}for (int k0 = 0; k0 < K; k0 += {bk_outer}) {{")
    lines.append(f"{pad}    threadgroup_barrier(mem_flags::mem_none);")
    if bk_outer > bk_inner:
        lines.append(f"{pad}    for (int k1 = 0; k1 < {bk_outer}; k1 += {bk_inner}) {{")
        lines.append(f"{pad}        const int k = k0 + k1;")
        lines.append(f"{pad}        ct_a.load(tA.template slice<{bk_inner}, {SM}>(k, tile_row));")
        lines.append(f"{pad}        ct_b.load(tB.template slice<{SN}, {bk_inner}>(tile_col, k));")
        lines.append(f"{pad}        op.run(ct_a, ct_b, cT);")
        lines.append(f"{pad}    }}")
    else:
        lines.append(f"{pad}    ct_a.load(tA.template slice<{bk_inner}, {SM}>(k0, tile_row));")
        lines.append(f"{pad}    ct_b.load(tB.template slice<{SN}, {bk_inner}>(tile_col, k0));")
        lines.append(f"{pad}    op.run(ct_a, ct_b, cT);")
    lines.append(f"{pad}}}")


def _emit_separated_pipelined(lines, pad, in_type, acc_type, SM, SN, bk_inner):
    """Emit double-buffered separated K-loop with 2 cooperative_tensor input sets.

    Flattens to a single loop over bk_inner (32-wide) tiles, alternating between
    buf0 and buf1. Each step: load next tile into bufN, compute current from bufM.
    This gives the hardware the chance to overlap loads with computes.

    Structure: prologue (load buf0) → main loop (step 2*bk_inner,
    load buf1+compute buf0, load buf0+compute buf1) → epilogue (drain odd).
    """
    step = bk_inner  # 32-wide tiles
    step2 = 2 * step
    p1 = pad + "    "
    p2 = pad + "        "

    lines.append(
        f"{pad}// Pipelined (2-stage): 2 cooperative_tensor input sets, overlapped load/compute"
    )
    _emit_ct_decl(lines, pad, in_type, suffix="0")
    _emit_ct_decl(lines, pad, in_type, suffix="1")
    lines.append(f"{pad}auto cT = op.get_destination_cooperative_tensor<")
    lines.append(f"{pad}    decltype(ct_a0), decltype(ct_b0), {acc_type}>();")
    _emit_ct_zero_init(lines, pad, acc_type)
    lines.append("")

    # Prologue: load first 32-wide tile into buf0
    lines.append(f"{pad}// Prologue: load first tile into buf0")
    _emit_ct_load(lines, pad, "0", "0", step, SM, SN)
    lines.append("")

    # Main loop: flat over 32-wide tiles, step by 2
    lines.append(f"{pad}for (int k = {step}; k < K; k += {step2}) {{")

    # Phase 1: load buf1, compute buf0
    _emit_ct_load(lines, p1, "1", "k", step, SM, SN)
    lines.append(f"{p1}op.run(ct_a0, ct_b0, cT);")

    # Phase 2: if more tiles, load buf0 + compute buf1; else just drain buf1
    lines.append(f"{p1}if (k + {step} < K) {{")
    _emit_ct_load(lines, p2, "0", f"k + {step}", step, SM, SN)
    lines.append(f"{p2}op.run(ct_a1, ct_b1, cT);")
    lines.append(f"{p1}}} else {{")
    lines.append(f"{p2}op.run(ct_a1, ct_b1, cT);")
    lines.append(f"{p1}}}")
    lines.append(f"{pad}}}")

    # Epilogue: drain buf0 for odd total_tiles (including single-tile case)
    lines.append(f"{pad}if ((K / {step}) & 1) {{")
    lines.append(f"{p1}op.run(ct_a0, ct_b0, cT);")
    lines.append(f"{pad}}}")


def _emit_tensor_ops_preemptive(func: mir.MFunction, gemm_op: mir.MTensorOpsGemm) -> str:
    """Preemptive mode: each SG independently handles its own subtile.

    Two paths:
    - Separated loads (SM <= 32 and SN <= 32): uses cooperative_tensor inputs
      with two-level K-loop, volatile compiler barrier, and threadgroup_barrier
      for load/compute pipelining. ~10-15% faster at large sizes.
    - Direct tensor views (SM > 32 or SN > 32): passes device tensor slices
      directly to op.run(). No dimension constraints.
    """
    BM, BN, BK = gemm_op.block_m, gemm_op.block_n, gemm_op.bk
    WM, WN = gemm_op.wm, gemm_op.wn
    SM, SN = gemm_op.sm, gemm_op.sn
    NUM_SG = gemm_op.num_sg
    in_type = gemm_op.in_type
    acc_type = gemm_op.acc_type
    relaxed = "true" if gemm_op.relaxed_precision else "false"

    # Use separated loads when descriptor dimensions allow cooperative_tensor inputs
    # (M, N, K all must be 16 or 32 for cooperative_tensor inputs)
    use_separated = SM <= 32 and SN <= 32
    bk_inner = min(32, BK) if use_separated else BK
    bk_outer = BK

    lines = _emit_tensor_ops_header(func, gemm_op, need_sgid=True)
    pad = "    "
    ...
    if gemm_op.swizzle == "morton":
        # Morton 2x2 block order: dispatch in 2x2 blocks for L2 reuse.
        # Falls back to linear dispatch for small grids where Morton
        # would map multiple threadgroups to the same tile.
    ...
    # Per-SG tile assignment
    lines.append(
        f"{pad}// {NUM_SG} preemptive simdgroups, {WM}x{WN} layout, each handles {SM}x{SN}"
    )
    lines.append(f"{pad}const uint sg_row = sgid / {WN}u;")
    lines.append(f"{pad}const uint sg_col = sgid % {WN}u;")
    lines.append(f"{pad}const uint tile_row = {row_expr} * {BM}u + sg_row * {SM}u;")
    lines.append(f"{pad}const uint tile_col = {col_expr} * {BN}u + sg_col * {SN}u;")
    lines.append("")

    # Descriptor — use bk_inner for separated loads path
    desc_bk = bk_inner if use_separated else BK
    ...
    if use_separated:
        num_stages = gemm_op.num_stages

        if num_stages >= 2:
            _emit_separated_pipelined(lines, pad, in_type, acc_type, SM, SN, bk_inner)
        else:
            _emit_separated_simple(lines, pad, in_type, acc_type, SM, SN, bk_inner, bk_outer)
    else:
        # Direct tensor view path: no cooperative_tensor input constraints
        lines.append(f"{pad}auto cT = op.get_destination_cooperative_tensor<")
        ...
        lines.append(f"{pad}#pragma clang loop unroll(full)")
        lines.append(f"{pad}for (uint16_t i = 0; i < cT.get_capacity(); ++i) {{")
        lines.append(f"{pad}    if (cT.is_valid_element(i)) cT[i] = {acc_type}(0);")
        lines.append(f"{pad}}}")
        lines.append("")

        lines.append(f"{pad}for (int k = 0; k < K; k += {BK}) {{")
        lines.append(f"{pad}    auto mA = tA.template slice<{BK}, {SM}>(k, tile_row);")
        lines.append(f"{pad}    auto mB = tB.template slice<{SN}, {BK}>(tile_col, k);")
        lines.append(f"{pad}    op.run(mA, mB, cT);")
        lines.append(f"{pad}}}")
    lines.append("")

    _emit_tensor_ops_epilogue(gemm_op, lines)
    lines.append("}")
    return "\n".join(lines)


def _emit_tensor_ops_cooperative(func: mir.MFunction, gemm_op: mir.MTensorOpsGemm) -> str:
    """Cooperative mode: all SGs work together on the full tile."""
    BM, BN, BK = gemm_op.block_m, gemm_op.block_n, gemm_op.bk
    NUM_SG = gemm_op.num_sg
    in_type = gemm_op.in_type
    acc_type = gemm_op.acc_type
    relaxed = "true" if gemm_op.relaxed_precision else "false"
    k_unroll = gemm_op.k_unroll

    lines = _emit_tensor_ops_header(func, gemm_op, need_sgid=False)
    pad = "    "
    ...
    if gemm_op.swizzle == "morton":
        ...
        row_expr = "tgp_id.x"
        col_expr_base = "tgp_id.y"
    col_expr = f"{col_expr_base} * {BN}u"
    row_expr_full = f"{row_expr} * {BM}u"
    lines.append("")

    # Descriptor — full tile dimensions, cooperative SGs
    lines.append(f"{pad}constexpr auto desc = matmul2d_descriptor(")
    lines.append(f"{pad}    {BM}, {BN}, {BK},")
    lines.append(f"{pad}    false, false, {relaxed},")
    lines.append(f"{pad}    matmul2d_descriptor::mode::multiply_accumulate);")
    lines.append(f"{pad}matmul2d<desc, execution_simdgroups<{NUM_SG}>> op;")
    lines.append("")

    # Output slice — full tile, static extents for compiler optimization
    lines.append(f"{pad}auto mC = tC.template slice<{BN}, {BM}>({col_expr}, {row_expr_full});")
    lines.append("")

    # cooperative_tensor init
    lines.append(f"{pad}auto cT = op.get_destination_cooperative_tensor<")
    lines.append(f"{pad}    tensor<device {in_type}, dextents<int32_t, 2>, tensor_inline>,")
    lines.append(
        f"{pad}    tensor<device {in_type}, dextents<int32_t, 2>, tensor_inline>, {acc_type}>();"
    ...
    # K-loop — static template slices for compiler optimization
    k_step = BK * k_unroll
    lines.append(f"{pad}for (int k = 0; k < K; k += {k_step}) {{")
    for u in range(k_unroll):
        k_off = f"k + {BK * u}" if u > 0 else "k"
        lines.append(
            f"{pad}    auto mA{u} = tA.template slice<{BK}, {BM}>({k_off}, {row_expr_full});"
        )
        lines.append(f"{pad}    auto mB{u} = tB.template slice<{BN}, {BK}>({col_expr}, {k_off});")
    for u in range(k_unroll):
        lines.append(f"{pad}    op.run(mA{u}, mB{u}, cT);")
    lines.append(f"{pad}}}")
    lines.append("")
...
def _emit_gemm_op(
    ...
    # --- Legacy fallback handlers (kept for backward compatibility) ---
    elif isinstance(op, mir.MAccumulatorInit):
        # Fully unroll accumulator init to avoid dynamic indexing
        lines.append(f"{pad}simdgroup_matrix<float, 8, 8> {op.acc_name}[{op.num_8m}][{op.num_8n}];")
        for mi in range(op.num_8m):
            for ni in range(op.num_8n):
                lines.append(
                    f"{pad}{op.acc_name}[{mi}][{ni}] = make_filled_simdgroup_matrix<float, 8, 8>(0.0f);"
                )
        ...
        _emit_mma_inner_loop(op, lines, indent, func)

    elif isinstance(op, mir.MAccumulatorEpilogue):
        _emit_accumulator_epilogue(op, lines, indent, func)

    elif isinstance(op, mir.MAccumulatorStore):
        _emit_accumulator_store(op, lines, indent, func)
...
def _emit_a_load(op, lines, pad, sg_row, mi_offset):
    """Emit A matrix simdgroup load (normal or swizzled)."""
    if op.a_swizzle_bits > 0:
        ...
        lines.append(
            f"{pad}simdgroup_load(a_mat, {op.shared_a} + ({sg_row} + {mi_offset}) * {op.a_stride} + kk, {op.a_stride});"
        )


def _emit_b_load(op, lines, pad, sg_col, ni_offset):
    """Emit B matrix simdgroup load (normal or swizzled)."""
    if op.b_swizzle_bits > 0:
        _emit_sg_load_swizzle(
            lines,
            pad,
            "b_mat",
        ...
        lines.append(
            f"{pad}simdgroup_load(b_mat, {op.shared_b} + kk * {op.b_stride} + ({sg_col} + {ni_offset}), {op.b_stride});"
        )


def _emit_mma_inner_loop(op: mir.MMAInnerLoop, lines: list[str], indent: int, func: mir.MFunction):
    """Emit the simdgroup MMA inner loop.

    When op.sg_layout is set, derives num_8m/num_8n from the layout algebra's
    simdgroup tiling (sg_m/8, sg_n/8) rather than legacy fields.

    When swizzle bits are set on A or B, replaces simdgroup_load with manual
    thread_elements() loading using XOR'd shared memory addresses for
    bank-conflict-free access.
    """
    pad = "    " * indent
    sg_row = _val_name_gemm(op.sg_row, func)
    sg_col = _val_name_gemm(op.sg_col, func)

    # Prefer layout-derived MMA tile counts when available
    if op.sg_layout is not None:
        num_8m = op.sg_layout.sg_m // 8
        num_8n = op.sg_layout.sg_n // 8
    else:
        num_8m = op.num_8m
        num_8n = op.num_8n

    lines.append(f"{pad}#pragma clang loop unroll(full)")
    lines.append(f"{pad}for (int kk = 0; kk < {op.bk}; kk += 8) {{")

    if op.preload_tiles:
        # Preloaded structure: load all A tiles, then all B tiles, then compute
        # (no swizzle support — preload_tiles causes register spilling anyway)
        lines.append(f"{pad}    simdgroup_matrix<{op.in_type}, 8, 8> a_mats[{num_8m}];")
        lines.append(f"{pad}    for (int mi = 0; mi < {num_8m}; mi++)")
        lines.append(
            f"{pad}        simdgroup_load(a_mats[mi], {op.shared_a} + ({sg_row} + mi * 8) * {op.a_stride} + kk, {op.a_stride});"
        )
        lines.append(f"{pad}    simdgroup_matrix<{op.in_type}, 8, 8> b_mats[{num_8n}];")
        lines.append(f"{pad}    for (int ni = 0; ni < {num_8n}; ni++)")
        lines.append(
            f"{pad}        simdgroup_load(b_mats[ni], {op.shared_b} + kk * {op.b_stride} + ({sg_col} + ni * 8), {op.b_stride});"
        )
        lines.append(f"{pad}    for (int mi = 0; mi < {num_8m}; mi++)")
        lines.append(f"{pad}        for (int ni = 0; ni < {num_8n}; ni++)")
        lines.append(
            f"{pad}            simdgroup_multiply_accumulate({op.acc_name}[mi][ni], a_mats[mi], b_mats[ni], {op.acc_name}[mi][ni]);"
        )
    else:
        # Interleaved structure with optional serpentine traversal
        barrier_str = (
            f"{pad}    simdgroup_barrier(mem_flags::mem_none);"
            if op.use_simdgroup_barrier
            else None
        )

        if op.serpentine:
            # Fully unrolled serpentine: even mi rows go forward, odd go backward
            for mi in range(num_8m):
                lines.append(f"{pad}    {{")
                if barrier_str:
                    lines.append(barrier_str)
                lines.append(f"{pad}        simdgroup_matrix<{op.in_type}, 8, 8> a_mat;")
                _emit_a_load(op, lines, f"{pad}        ", sg_row, mi * 8)
                n_order = range(num_8n) if mi % 2 == 0 else range(num_8n - 1, -1, -1)
                for ni in n_order:
                    lines.append(f"{pad}        {{")
                    if barrier_str:
                        lines.append(f"{pad}            simdgroup_barrier(mem_flags::mem_none);")
                    lines.append(f"{pad}            simdgroup_matrix<{op.in_type}, 8, 8> b_mat;")
                    _emit_b_load(op, lines, f"{pad}            ", sg_col, ni * 8)
                    if barrier_str:
                        lines.append(f"{pad}            simdgroup_barrier(mem_flags::mem_none);")
                    lines.append(
                        f"{pad}            simdgroup_multiply_accumulate({op.acc_name}[{mi}][{ni}], a_mat, b_mat, {op.acc_name}[{mi}][{ni}]);"
                    )
                    lines.append(f"{pad}        }}")
                lines.append(f"{pad}    }}")
        else:
            # Fully unroll MMA to keep accumulators in registers
            for mi in range(num_8m):
                lines.append(f"{pad}    {{")
                if barrier_str:
                    lines.append(barrier_str)
...
def _emit_accumulator_store(
    op: mir.MAccumulatorStore, lines: list[str], indent: int, func: mir.MFunction
):
    """Emit simdgroup_store of accumulators to device memory."""
    pad = "    " * indent
    ptr = _val_name_gemm(op.device_ptr, func)
    ...
    # Fully unroll accumulator store to avoid dynamic indexing of simdgroup_matrix
    for mi in range(op.num_8m):
        for ni in range(op.num_8n):
            lines.append(f"{pad}{{")
            lines.append(f"{pad}    uint _or = {br} + {sr} + {mi * 8};")
            lines.append(f"{pad}    uint _oc = {bc} + {sc} + {ni * 8};")
            ...
            if op.out_type == "half" and op.acc_type == "float":
                ...
                lines.append(
                    f"{pad}        _out.thread_elements()[0] = half({op.acc_name}[{mi}][{ni}].thread_elements()[0]);"
                )
                lines.append(
                    f"{pad}        _out.thread_elements()[1] = half({op.acc_name}[{mi}][{ni}].thread_elements()[1]);"
                )
                lines.append(
                    f"{pad}        simdgroup_store(_out, {ptr} + _or * {stride} + _oc, {stride});"
                )
            else:
                lines.append(
                    f"{pad}        simdgroup_store({op.acc_name}[{mi}][{ni}], {ptr} + _or * {stride} + _oc, {stride});"
                )
            lines.append(f"{pad}    }}")
            lines.append(f"{pad}}}")


def _emit_accumulator_epilogue(
    op: mir.MAccumulatorEpilogue, lines: list[str], indent: int, func: mir.MFunction
):
    """Emit element-wise epilogue on simdgroup_matrix accumulators via thread_elements().

    Each thread owns 2 elements of each 8x8 simdgroup_matrix tile.
    Operations are applied directly in registers — zero memory traffic.
    """
    pad = "    " * indent
    acc = op.acc_name
    lines.append(
        ...
        for ni in range(op.num_8n):
            for epi in op.operations:
                if epi[0] == "relu":
                    ...
                    # Scale by a compile-time factor (for now, just emit as-is)
                    for e in (0, 1):
...
def _emit_double_buffered_k_loop(
    ...
    # Support both legacy MMAInnerLoop and decomposed kk ForLoop
    mma_ops = [o for o in op.body if isinstance(o, mir.MMAInnerLoop)]
    kk_loops = [o for o in op.body if isinstance(o, mir.MForLoop) and getattr(o, "_unroll", False)]
    compute_ops = mma_ops or kk_loops
    if not loads or not compute_ops:
    ...
    if mma_ops:
        # Legacy path: MMAInnerLoop
        mma = mma_ops[0]
        old_sa = mma.shared_a
        old_sb = mma.shared_b
        mma.shared_a = "sa_curr"
        mma.shared_b = "sb_curr"
        _emit_mma_inner_loop(mma, lines, indent + 1, func)
        mma.shared_a = old_sa
        mma.shared_b = old_sb
    else:
        # Decomposed path: kk ForLoop with MSimdgroupLoad/MMA
        kk = kk_loops[0]
    ...
    if mma_ops:
        ...
        mma.shared_a = "sa_curr"
        mma.shared_b = "sb_curr"
        _emit_mma_inner_loop(mma, lines, indent, func)
        mma.shared_a = old_sa
        mma.shared_b = old_sb
    else:
        kk = kk_loops[0]
    ...
    for body_op in op.body:
        if isinstance(body_op, mir.MSimdgroupRoleBlock):
            ...
            if is_producer:
                ...
                for inner_op in body_op.body:
                    if isinstance(inner_op, mir.MMAInnerLoop):
                        # Legacy: Replace shared_a/shared_b with sa_curr/sb_curr
                        orig_a = inner_op.shared_a
                        orig_b = inner_op.shared_b
                        inner_op.shared_a = "sa_curr"
                        inner_op.shared_b = "sb_curr"
                        _emit_gemm_op(inner_op, lines, indent + 2, func, has_swizzle)
                        inner_op.shared_a = orig_a
                        inner_op.shared_b = orig_b
                    elif isinstance(inner_op, mir.MForLoop) and getattr(inner_op, "_unroll", False):
