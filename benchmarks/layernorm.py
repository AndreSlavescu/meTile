...
autotuned_layernorm = metile.autotune(
    configs=LAYERNORM_CONFIGS,
    key=["N"],
    verbose=True,
)(layernorm)
...
def main():
    ...
    for nrows, hidden in [(128, 512), (256, 1024), (512, 2048), (1024, 4096)]:
        ...
        grid = (nrows,)
        dispatch = autotuned_layernorm[grid].prepare(X_buf, W_buf, B_buf, Out_buf, hidden)

        dev.sync()

        ln_mlx = nn.LayerNorm(hidden)
        ln_mlx.weight = mx.array(W_np)
        ln_mlx.bias = mx.array(B_np)
        X_mx = mx.array(X_np)

        def mlx_fn(norm=ln_mlx, x=X_mx):
            mx.eval(norm(x))
    ...
    _print_table("layernorm vs MLX", rows)
    print()
